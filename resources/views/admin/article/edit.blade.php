<x-layouts.admin title="Edit Artikel" description="Edit artikel">
    <div class="space-y-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Artikel</h1>
                <p class="text-gray-600 dark:text-gray-400">Edit artikel "{{ $article->title }}"</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <form id="article-form" action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <textarea id="excerpt" name="excerpt" rows="3" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('excerpt', $article->excerpt) }}</textarea>
                            @error('excerpt')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <textarea id="content" name="content" rows="10" class="hidden">{{ old('content', $article->content) }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Kategori Tambahan</label>
                        <div id="additional-categories-container" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                            @foreach($categories as $category)
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $article->is_featured) ? 'checked' : '' }}
                                   class="rounded border-gray-300 dark:border-gray-600 text-green-600 focus:ring-green-500 dark:bg-gray-700">
                            <label for="is_featured" class="ml-2 text-sm text-gray-700 dark:text-gray-300">Featured</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" id="is_trending" name="is_trending" value="1" {{ old('is_trending', $article->is_trending) ? 'checked' : '' }}
                                   class="rounded border-gray-300 dark:border-gray-600 text-green-600 focus:ring-green-500 dark:bg-gray-700">
                    ...
                    <div>
                        ...
                        <input type="text" id="published_at" name="published_at" value="{{ old('published_at', $article->published_at ? $article->published_at->format('Y-m-d H:i') : '') }}"
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="meta_description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Meta
                                    Description</label>
                                <input type="text" id="meta_description" name="meta_description" value="{{ old('meta_description', $article->meta_description) }}"
                ...
                <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <a href="{{ route('admin.articles.index') }}" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-medium rounded-lg transition-colors">
                        Batal
                    </a>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.getElementById('article-form').addEventListener('submit', function(e) {
                    ...
                    if (!contentValue) {
                        ...
                        submitButton.innerHTML = 'Update Artikel';
                    ...
                    fetch(form.action, {
                        ...
                        .then(data => {
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex space-x-2">
                                    <a href="${data.redirect || '{{ route('admin.articles.show', $article) }}'}" class="text-green-700 hover:text-green-800 underline text-sm">Lihat Artikel</a>
                                    <button type="button" onclick="window.location.reload()" class="text-green-700 hover:text-green-800 underline text-sm">Edit Lagi</button>
                                </div>
                            </div>`;
                                form.reset();
                                if (ckeditorInstance) {
                ...
                // --- Tampilkan gambar yang sudah ada saat load ---
                @if($article->image)
